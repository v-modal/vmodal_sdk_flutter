...
import 'user/mock_firebase_auth.dart';
import 'user/user_session_controller.dart';
import 'user/vmodal_credential.dart';
...
class FramebaseApp extends StatefulWidget {
  const FramebaseApp({super.key, this.controller, this.session});
...
class _FramebaseAppState extends State<FramebaseApp> {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
    ...
    builder: (context, _) => MaterialApp(
      ...
      home: session.state == SessionState.ready && session.canRead
          ? LibraryPage(controller: archive, session: session)
...
class LibraryPage extends StatefulWidget {
  const LibraryPage({super.key, this.controller, required this.session});
  final ArchiveController? controller;
  final UserSessionController session;
  @override
    ...
    builder: (context, _) => Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            tooltip: 'Add video',
            onPressed: archive.busy || !widget.session.canWrite
                ? null
                : importVideo,
          ...
          PopupMenuButton<String>(
            ...
            onSelected: (value) {
              if (value == 'profile') {
                showModalBottomSheet<void>(
                  context: context,
                  builder: (_) => ProfileSheet(session: widget.session),
                );
              }
              if (value == 'history') {
                Navigator.push(
            ...
            itemBuilder: (_) => [
              ...
              PopupMenuItem(
                value: 'prepare',
                enabled: widget.session.canWrite,
                child: const Text('Prepare videos for search'),
              ),
              const PopupMenuItem(
                value: 'history',
        ...
        children: [
          ...
          if (archive.connected && !archive.ready && !archive.busy) ...[
            ...
            TextButton.icon(
              ...
              onPressed: widget.session.canWrite
                  ? () => prepareArchive(context, archive)
                  : null,
      ...
      bottomNavigationBar: SafeArea(
        child: Padding(
          ...
          child: SearchDock(onTap: archive.ready ? openSearch : null),
        ),
      ),
    ),
  );
...
class _SearchPageState extends State<SearchPage> {
  ...
  Future<void> search([String? text]) async {
    ...
    if (!archive.connected || !archive.canRead) return;
    if (!archive.ready) {
      return;
    }
